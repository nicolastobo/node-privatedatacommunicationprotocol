...
function hmac (k, d) {
  return crypto.createHmac('sha1', k)
               .update(d)
               .digest();
};
...
PDCP.crypt = function (e_key, i_key, price, iv) {
  ...
  if(! Buffer.isBuffer(e_key))
    throw('Invalid e_key is not a Buffer');
  if(e_key.length != 32)
    throw('Invalid e_key is not 32 bytes');


  if(! Buffer.isBuffer(i_key))
    throw('Invalid i_key is not a Buffer');
  if(i_key.length != 32)
    throw('Invalid i_key is not 32 bytes');

  if (iv) {
    if(! Buffer.isBuffer(iv))
      throw('Invalid iv is not a Buffer');
    if(iv.length != 16)
      throw('Invalid iv is not 16 bytes');
  } else {
    iv = new Buffer(16);
    iv.writeUInt16BE(Math.round(Math.random() * 65535), 0);
  }

  var bprice = new Buffer(32);
  bprice.writeUInt32BE(price, 4);

  var pad     = hmac(e_key, iv).slice(0, 8),
    enc_price = xor(pad, bprice),
    conf_sig  = hmac(i_key, Buffer.concat([bprice.slice(0, 8), iv])).slice(0, 4),
    result    = Buffer.concat([iv, enc_price, conf_sig]);
...
PDCP.decrypt = function (e_key, i_key, encodedPrice) {
  if (! URLSafeBase64.validate(encodedPrice))
    throw('encodedPrice is not a URLSafeBase64');
